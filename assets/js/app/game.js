...
game.drawTextGame = function(gameState){
...
game.move = function(initialState, move){
  if (game.state.view.isBoardCreated() && !game.state.view.boardIsText()){
...
game.events.spaceClicked = function(x,y) {
...
game.events.moveAttempt = function(source, target) {
...
game.events.updateState = function(state) {
...
game.events.newMatch = function(state) {
...
game.init = {};
  ...
  */
game.init.board = function(cb, options){
...
game.end.board =  function(options){
  if (options == undefined){
    options = {};
...
game.state.view.isBoardCreated = function(){
  return d3.select("#game").node() != null;
};

game.state.view.boardIsText = function(){
...
game.state.view.piecePath = "./images/pieces/";

// MODEL

// Contains the data for the view
game.state.model = {};
game.state.model.getSpace = function(x, y) {
  if(!game.state.model.match.board)
    return undefined;
  else
    return game.state.model.match.board[y][x];
};
game.state.model.removeHighlights = function() {
